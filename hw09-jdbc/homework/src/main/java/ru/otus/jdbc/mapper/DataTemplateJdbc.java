...
public class DataTemplateJdbc<T> implements DataTemplate<T> {
    ...
    public long insert(Connection connection, T entity) {
        try {
            return dbExecutor.executeStatement(connection, entitySQLMetaData.getInsertSql(), paramsToExecute(entity));
        } catch (Exception ex) {
            throw new DataTemplateException(ex);
        }
    }
    ...
    public void update(Connection connection, T entity) {
        try {
            var list = paramsToExecute(entity);
    ...
    private List<Object> paramsToExecute(T entity) throws Exception {
        ...
        for (int i = 0; i < fields.size(); i++) {
            ...
            if (!str.isEmpty()) {
                params[i] = (entity.getClass().getDeclaredMethod(str).invoke(entity));
            }
        }
